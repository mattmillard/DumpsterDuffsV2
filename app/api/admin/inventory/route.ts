...
export async function PUT(request: Request) {
  try {
    ...
    if (!Number.isFinite(totalUnits) || !Number.isFinite(availableUnits)) {
      return NextResponse.json(
        { error: "total_units and available_units must be valid numbers" },
        { status: 400 },
      );
    }
    ...
    if (availableUnits > totalUnits) {
      return NextResponse.json(
        { error: "Available units cannot exceed total units" },
        { status: 400 },
      );
    }
    ...
    if (existingIndex === -1) {
      return NextResponse.json(
        { error: `Inventory item not found for id: ${body.id}` },
        { status: 404 },
      );
    }
