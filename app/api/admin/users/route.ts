...
export async function GET() {
	try {
		...
		const [{ data: adminRows, error: adminRowsError }, usersResult] = await Promise.all([
			supabaseAdmin.from("admin_users").select("id, email, full_name, role, is_active, created_at, last_login"),
		...
		const adminMap = new Map((adminRows || []).map((row) => [row.id, row]));
		...
		const users = (usersResult.data?.users || [])
			.map((user) => {
				...
				const canPromote =
					!adminRecord || !adminRecord.is_active || (adminRecord.role !== "admin" && adminRecord.role !== "owner");
...
export async function PATCH(request: NextRequest) {
	try {
		...
		const targetRole: AdminRole = existingAdmin?.role === "owner" ? "owner" : "admin";
		...
		return NextResponse.json({
			success: true,
			user: upserted,
			message: targetRole === "owner" ? "Owner role preserved" : "User promoted to admin",
		});
	} catch (error) {
